...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, time_emb_dim, up=False):
        ...
        super().__init__()
        self.time_mlp = nn.Linear(time_emb_dim, out_channels) #时间嵌入层
        #第一次卷积扩展通道数，然后加入时间嵌入
        if up:  #上采样
            self.conv1 = nn.Conv2d(2 * in_channels, out_channels, kernel_size=3, padding=1) # 上一层的输出会和当前层的输入合并
            self.transform = nn.ConvTranspose2d(out_channels, out_channels, kernel_size=4, stride=2, padding=1) #通过步幅大于1的卷积操作将图像尺寸放大
        else:
            self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1) # 该层只需要处理当前层的特征图
            self.transform = nn.Conv2d(out_channels, out_channels, kernel_size=4, stride=2, padding=1)

        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1) #第二次卷积，融合特征信息
...
class SimpleUnet(nn.Module):
    ...
    def forward(self, x, time_step):
        ...
        for down in self.downs:
            x = down(x, t) #每个block接收当前的特征图x和时间嵌入t
            residual_stack.append(x)
        for up in self.ups:
            residual_x = residual_stack.pop()
            x = torch.cat((x, residual_x), dim=1) #拼接
            x = up(x, t)
