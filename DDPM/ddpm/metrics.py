...
class InceptionStatistics:
    def __init__(self, device='cuda'):
        ...
        self.model.fc = nn.Identity()  # 移除最后的全连接层，移除分类曾，输出特征向量
        self.model = self.model.to(device)
        self.model.eval()
        
        # 设置图像预处理
        self.preprocess = transforms.Compose([
            transforms.Resize(299), #Inception v3 需要299*299输入
    ...
    def get_features(self, images):
        ...
        for (batch,) in tqdm(dataloader):
            ...
            prob = F.softmax(feature, dim=1) #计算类别概率
...
def calculate_inception_score(probs, splits=10):
    ...
    probs:所有图像通过Inception v3的类别概率 (num_samples, num_classes)
    ...
    for i in tqdm(range(splits)):
        # 获取当前split的概率分布 p(y|x)
        part = probs[i * split_size:(i + 1) * split_size]
        # 计算KL散度: KL(p(y|x) || p(y))=p(y|x)*(logp(y|x)-logp(y))
